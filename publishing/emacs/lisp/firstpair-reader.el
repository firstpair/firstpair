...
(defun firstpair-reader--selection-state-slot (selections slot)
  "Convert 1.39 SELECTIONS to the bounded alist for zero-based SLOT."
  (delq nil
        (mapcar (lambda (pair)
                  (let ((id (nth slot (cdr pair))))
                    (and id (cons (car pair) id))))
                selections)))
...
(defun firstpair-reader--resume (bundle)
  ...
  (let ((state (and firstpair-reader-resume (firstpair-reader--state-of bundle))))
    (when state
      ...
      (let ((selections (plist-get state :selections)))
        (setq firstpair-reader-translation-choices
              (or (plist-get state :choices)
                  (firstpair-reader--selection-state-slot selections 0))
              firstpair-reader-second-translations
              (or (plist-get state :seconds)
                  (firstpair-reader--selection-state-slot selections 1))))
      (condition-case nil
          (progn
