...
(defun firstpair-reader--saved-selections (state)
  "Return ordered translation selections from current or legacy STATE.
Reader 1.39 and later store `:selections'.  Readers 1.38 and 1.40 stored one
`:choices' entry and an optional `:seconds' entry per language; preserve both
as checked editions when upgrading."
  (or (copy-tree (plist-get state :selections))
      (let (selections)
  ...
  (let ((state (and firstpair-reader-resume (firstpair-reader--state-of bundle))))
    (when state
      ...
      (setq firstpair-reader-translation-selections
            (firstpair-reader--saved-selections state)
